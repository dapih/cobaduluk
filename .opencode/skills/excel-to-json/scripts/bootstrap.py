...
Usage (from user project root — recommended):
    python excel-to-json/skills/excel-to-json/scripts/bootstrap.py
    python excel-to-json/skills/excel-to-json/scripts/bootstrap.py --agents cursor,kilo

Usage (clone + setup):
    python excel-to-json/skills/excel-to-json/scripts/bootstrap.py --clone --dest excel-to-json
...
MARKER = Path("skills/excel-to-json/scripts/resolve_plugin_root.py")
...
def bootstrap(
    ...
    *,
    ...
    if not skip_verify:
        verify_cmd = [
            sys.executable,
            str(plugin_root_path / "skills" / "excel-to-json" / "scripts" / "verify_install.py"),
            "--root",
...
def print_next_steps(
    ...
    print()
    print("Plugin root:")
    print(f"  python {rel}/skills/excel-to-json/scripts/resolve_plugin_root.py")
    if is_windows:
...
def main() -> None:
    ...
    parser.add_argument(
        "--root",
        help="Plugin root if already cloned (default: repo root, auto-detected)",
    )
    parser.add_argument(
        "--dest",
    ...
    if args.clone:
        ...
        print(
            ...
            f"python {DEFAULT_DEST}/skills/excel-to-json/scripts/bootstrap.py",
            file=sys.stderr,
        )
        sys.exit(1)
