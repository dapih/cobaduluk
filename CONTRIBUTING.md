...
skills/excel-to-json/      canonical skill: self-contained, portable via a skill-only install
  SKILL.md                   entry point
  references/                 just-in-time reference docs (schema design, parsing, reuse, ...)
  scripts/                    pipeline: inspect, validate, dq_check, bootstrap, install_adapters, ...
...
design/                    reuse (design history), roadmap, cross-tool-compat, token analysis
...
```
python skills/excel-to-json/scripts/inspect_xlsx.py   <file.xlsx> [--sheet NAME] [--out PREFIX]
python skills/excel-to-json/scripts/validate_json.py  <schema.json> <instance.json> [--counts]
python skills/excel-to-json/scripts/dq_check.py       <instance.json> [--rules rules.json] [--out PREFIX]
python skills/excel-to-json/scripts/fingerprint.py    <job.inspect.json> [--out PREFIX]
python skills/excel-to-json/scripts/match_profile.py  <job.inspect.json> [--families DIR] [--top N]
python skills/excel-to-json/scripts/promote_family.py <job-id> --name <family> [--force] [--evolve]
python skills/excel-to-json/scripts/conformance.py    <job.inspect.json> --name <family> [--job-schema PATH]
python skills/excel-to-json/scripts/learnings.py      --tags structure,tooling
```

## Key docs

- `skills/excel-to-json/workflows/full-pipeline.md` canonical step order, gates, tool assignments
- `skills/excel-to-json/references/reuse.md` fingerprint, families, tiers, conformance (operative reference)
- `design/reuse.md` reuse design rationale, spike results, build history
...
## Before changing a script in `skills/excel-to-json/scripts/`

1. Read `skills/excel-to-json/workflows/full-pipeline.md` for pipeline step callers.
...
4. If the token budget is affected: run `python skills/excel-to-json/scripts/measure_tokens.py <job>` before and after.
5. After structural changes: run `python skills/excel-to-json/scripts/validate_marketplace.py` and `python skills/excel-to-json/scripts/verify_install.py`.
...
```bash
python skills/excel-to-json/scripts/learnings.py --lint --entry '<entry>'
```
...
python skills/excel-to-json/scripts/link_skill_discovery.py      # if discovery mirrors missing (dev checkout only)
python skills/excel-to-json/scripts/validate_marketplace.py
python skills/excel-to-json/scripts/verify_install.py
python skills/excel-to-json/scripts/smoke_test_compat.py          # includes nested bootstrap gate
```
