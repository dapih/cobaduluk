...
Matching is by structural fingerprint, not headers (see `skills/excel-to-json/references/reuse.md`). Act on the verdict — **always confirm before reusing**:
...
`python "$PLUGIN_ROOT/skills/excel-to-json/scripts/conformance.py" output/<job>/<job>.inspect.json --name <family> --job-schema output/<job>/<job>.schema.json`
