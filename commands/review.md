...
Precondition: `output/<job>/<job>.json` exists (ideally already schema-valid).

Delegate to the **dq-reviewer** agent to:
1. run `dq_check.py --out output/<job>/<job>`;
2. interpret each finding as a real error, a cleanup, or a source characteristic (spot-check the source);
3. write `output/<job>/data-quality-<job>.md` from the template, including the row-conservation line and a prioritized **Recommendations** section (proposed, not applied — including the standing `-`/blank → empty suggestion);
