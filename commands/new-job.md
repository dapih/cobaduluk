...
4. Start the log: copy `${CLAUDE_PLUGIN_ROOT}/templates/log.md` to `output/<job-id>/log-<job-id>.md` and fill the header (job id, source file, sheet if known, start datetime, status `in-progress`). Add a first milestone line.
