---
description: Create a new conversion job folder under docs/, move/copy the Excel input in, and start its log.
...
1. Determine the **job id**:
   ...
   - Else build `table-YYYYMMDD-HHMM<am|pm>` from the **moment the job is created** (local time): the `YYYYMMDD` date plus the 12-hour clock time as zero-padded `HHMM` with a lowercase `am`/`pm` suffix — e.g. 7:30 PM on 2026-06-28 → `table-20260628-0730pm`. Get both at once with `date +%Y%m%d-%I%M%P` (GNU `%P` yields lowercase `am`/`pm`; if `%P` is unavailable use `%p` and lowercase the result). In the rare event that `docs/<that-id>/` already exists (a second job within the same minute), append `-2`, `-3`, … to disambiguate.
2. Create `docs/<job-id>/`.
...
4. Start the log: copy `${CLAUDE_PLUGIN_ROOT}/templates/log.md` to `docs/<job-id>/log-<job-id>.md` and fill the header (job id, source file, sheet if known, start datetime, status `in-progress`). Add a first milestone line.
