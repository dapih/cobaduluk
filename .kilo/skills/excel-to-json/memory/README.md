...
`python "${CLAUDE_PLUGIN_ROOT}/skills/excel-to-json/scripts/learnings.py" --tags <tags>` — structure-analyst → `structure,tooling`; schema-designer → `schema,structure,tooling`; parser-builder → `normalization,structure,tooling`; dq-reviewer → `dq,tooling`. Apply entries whose Context matches the table; ignore the rest.
...
`python "${CLAUDE_PLUGIN_ROOT}/skills/excel-to-json/scripts/learnings.py" --lint --entry '<entry>'`
