...
overview: Move scripts/, skill-rules/, templates/, workflows/, and the operative half of design/reuse.md inside skills/excel-to-json/, so a generic "grab the SKILL.md folder" installer (npx skills add / skills.sh) produces a fully working skill, not a stub.
todos:
  - id: preflight
  ...
  - id: split-reuse
    ...
    status: pending
  - id: memory-optional
    content: "Optional: move memory/ into skills/excel-to-json/memory/; update learnings.py path + references"
    status: pending
  - id: verify-mirrors
...
`skills.sh` (`npx skills add`) and similar generic skill indexers only capture the folder containing `SKILL.md`, here `skills/excel-to-json/`. Everything the pipeline needs to actually *run* (`scripts/`, `skill-rules/`, `templates/`, `workflows/`) currently lives outside that folder, at the repo root. A plain `npx skills add` install therefore produces a skill that can describe the pipeline but can't execute it. This is already a known, documented gap (`INSTALL.md` and `skills/README.md` both warn "not sufficient alone"); this plan fixes the root cause instead of warning around it.

The earlier objection to nesting everything ("it'll duplicate the engine into every agent's mirrored folder") doesn't hold up. `install_adapters.py` links (`link_directory`, symlink/junction, copy only as fallback) the *whole* `skills/excel-to-json/` tree per agent already, so nesting `scripts/` etc. inside it costs nothing under the normal bootstrap install: a junction is just another path to the same files. The only place it has a real cost is the copy-fallback path and the `npx skills add` path, and making the latter actually work is the entire point.

## What moves in

| Folder | Destination | Why |
|---|---|---|
| `scripts/` | `skills/excel-to-json/scripts/` | Load-bearing at execution time; the skill can't run without it regardless of install method |
| `skill-rules/` | `skills/excel-to-json/skill-rules/` | Two small JSON configs scripts load at runtime; same reasoning as scripts/ |
...
| `memory/` (optional, lower priority) | `skills/excel-to-json/memory/` | Mutable per-install state, not skill definition; moving it is harmless but not required for the fix |
...
- `scripts/resolve_plugin_root.py`: the `MARKER` constant (see above)
- `scripts/bootstrap.py`, `scripts/install_adapters.py`: verify, don't assume, that no code hardcodes `scripts/`/`skill-rules/`/`templates/`/`workflows/` as repo-root-relative outside of what moving `CANONICAL_SKILL`'s contents already handles
- `scripts/verify_install.py`, `scripts/validate_marketplace.py`, `scripts/smoke_test_compat.py`: required-directory checks need their expected paths updated
...
- Update all `$PLUGIN_ROOT/scripts/...` references (see scope list)
- Re-run checklist
...
### Phase 6 — Move `memory/` (optional)
- `git mv memory skills/excel-to-json/memory`
- Update `learnings.py`'s path resolution and every `memory/learnings.md` / `memory/README.md` reference
- Re-run checklist
- Can be skipped or deferred without undermining the rest of the migration; it's mutable state, not skill definition

### Phase 7 — Confirm mirroring needs no code change
- `install_adapters.py` already links the entire `skills/excel-to-json/` tree per agent (`CANONICAL_SKILL = Path("skills/excel-to-json")`); it never mirrored `scripts/` separately. Verify (don't assume) this holds by running `smoke_test_compat.py`'s full matrix and checking each agent's mirrored folder actually contains the new subfolders.
...
- **The copy-fallback install path** (`EXCEL_TO_JSON_FORCE_COPY=1`, or a failed junction) will genuinely duplicate the engine into each agent's mirror once `scripts/` moves in. This is a real, small, deliberately accepted cost that only affects a fallback path, not the primary symlink/junction-based install.
