...
## Resume guide (session paused — implementation not started)

**Status:** Planning complete · **No code or repo changes made yet** · All todos still `pending`

| Item | Location |
|---|---|
| **Plan (canonical for Cursor)** | `C:\Users\Davi Muammar\.cursor\plans\cross-tool_skills_migration_fa49cabf.plan.md` |
| **Repo copy (created on M0)** | `design/plans/cross-tool-skills-migration.md` — *does not exist yet* |
| **Progress tracker (created on M0)** | `design/cross-tool-compat.md` — *does not exist yet* |
| **Workspace** | `c:\Works\AI works & experiments\cobaduluk` |
| **GitHub target** | `dapih/cobaduluk` |

**To resume in a new chat**, paste:

> Continue implementing the cross-tool skills migration plan for excel-to-json. Read `design/plans/cross-tool-skills-migration.md` (or `.cursor/plans/cross-tool_skills_migration_fa49cabf.plan.md` if not copied yet). Start at **M0** if nothing is done; otherwise read `design/cross-tool-compat.md` for current milestone status and pick up from there.
...
**Start here when implementing:** M0 → copy plan to repo + scaffold `design/cross-tool-compat.md` → M1 marketplace manifest

**Nothing committed:** safe to stop; repo is unchanged from planning phase.
...
/plugin install excel-to-json@dapih-plugins
```
...
|---|---|---|
| **Claude Code** | `/plugin marketplace add dapih/cobaduluk` | `/plugin install excel-to-json@dapih-plugins` |
